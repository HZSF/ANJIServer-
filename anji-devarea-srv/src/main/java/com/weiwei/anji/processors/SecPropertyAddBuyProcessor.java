...
public class SecPropertyAddBuyProcessor extends BaseProcessor {
	private String username;
	private PropertyAddBuyRequest request;
	private PropertyDAOImpl propertyDao;
	@Override
	protected void preProcess(Map scopes){
		username = (String)scopes.get(Constants.USERNAME);
		request = (PropertyAddBuyRequest)scopes.get(Constants.SERVICE_REQUEST);
	...
	protected String executeProcess(Map scopes) {
		try{
			...
			propertyDao.insertBuyProperty(username, region, category, property_area, levels, ask_price, description);
