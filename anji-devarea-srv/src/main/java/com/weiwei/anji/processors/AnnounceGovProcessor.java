...
public class AnnounceGovProcessor extends BaseProcessor{
	...
	protected String executeProcess(Map scopes) {
		...
		if(request.getUrl() != null && !"".equalsIgnoreCase(request.getUrl().trim())){
			...
			if(Integer.parseInt(request.getEndNumber()) >= Integer.parseInt(request.getStartNumber())){
				announcementList_db = announceDao.findGovBySequenceId(Integer.parseInt(request.getStartNumber()), Integer.parseInt(request.getEndNumber()));
