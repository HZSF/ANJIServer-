...
public class AnnounceNewGovProcessor extends BaseProcessor{
	...
	protected String executeProcess(Map scopes) {
		...
		if(request.getUrl() != null && !"".equalsIgnoreCase(request.getUrl().trim())){
			announcementList_db = announceDao.findGovByUrl(request.getUrl());
			...
			if(Integer.parseInt(request.getEndNumber()) >= Integer.parseInt(request.getStartNumber())){
				announcementList_db = announceDao.findGovBySequenceIdNew(Integer.parseInt(request.getStartNumber()), Integer.parseInt(request.getEndNumber()));
